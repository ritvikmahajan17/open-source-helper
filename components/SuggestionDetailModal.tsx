  ...
  return parts.map((part, index) => {
    if (part.startsWith("`") && part.endsWith("`")) {
      return (
        <code
          key={index}
          className="bg-gray-800 text-red-400 px-1.5 py-1 rounded-md text-xs font-mono"
        >
          {part.slice(1, -1)}
        </code>
      );
...
export const SuggestionDetailModal: React.FC<SuggestionDetailModalProps> = ({
  ...
  useEffect(() => {
    if (isOpen && !details) {
      // Only fetch if modal is open and we don't have details yet
      ...
      <div className="bg-black border border-gray-800 rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col animate-in fade-in-0 zoom-in-95">
          ...
          <div>
            ...
            {suggestion.issueUrl ? (
              <p className="text-sm text-gray-400 mt-1">
                {renderWithCodeSnippets(suggestion.description)}
              </p>
            ) : (
        ...
        <div className="p-6 overflow-y-auto">
          ...
          {details && (
            <div className="space-y-6">
              <div>
                <h4 className="font-semibold text-lg mb-2 flex items-center gap-2 text-white">
                  <GitBranch className="w-5 h-5" /> How to Get Started
                </h4>
                <ul className="list-disc list-inside space-y-1 pl-2 text-gray-300">
                  {details.howToStart.map((step, i) => (
                    <li key={i}>{renderWithCodeSnippets(step)}</li>
                  ))}
                </ul>
              </div>
              <div>
                <h4 className="font-semibold text-lg mb-2 flex items-center gap-2 text-white">
                  <Code className="w-5 h-5" /> Potential Files to Edit
                </h4>
                <ul className="list-disc list-inside space-y-1 pl-2 text-gray-300">
                  {details.filesToEdit.map((file, i) => (
                    <li key={i}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <h4 className="font-semibold text-lg mb-2 flex items-center gap-2 text-white">
                    <i data-lucide="check-circle" className="w-5 h-5" /> Skills
                    Needed
                  </h4>
                  <div className="flex flex-wrap gap-2">
                    {details.skillsNeeded.map((skill, i) => (
                      <span
                        key={i}
                ...
                <div>
                  <h4 className="font-semibold text-lg mb-2 flex items-center gap-2 text-white">
                    <i data-lucide="clock" className="w-5 h-5" /> Estimated Time
                  </h4>
                  <p className="text-gray-300">
                    {details.estimatedHours} hours
                  </p>
                </div>
              </div>
              ...
              {details.sources && details.sources.length > 0 && (
                <div>
                  <h4 className="font-semibold text-lg mb-2 flex items-center gap-2 text-white">
                    <i data-lucide="link" className="w-5 h-5" /> Helpful Sources
                  </h4>
        ...
        <footer className="p-4 border-t border-gray-800 flex justify-end items-center gap-3 flex-shrink-0">
          ...
          {suggestion.issueUrl && (
            ...
            >
              Contribute on GitHub <ArrowUpRight className="w-4 h-4" />
            </a>
          )}
        </footer>
      </div>
